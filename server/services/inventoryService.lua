...
InventoryService.giveMoneyToPlayer = function(target, amount)
	...
	if not SvUtils.InProcessing(_source) then
		...
		elseif sourceMoney < amount then
			...
			TriggerClientEvent("vorp:TipRight", _source, string.format(T.YouPaid, tostring(amount), "ID: " .. _target), 3000)
			TriggerClientEvent("vorp:TipRight", _target, string.format(T.YouReceived, tostring(amount), "ID: " .. _source), 3000)
			Wait(3000)
			TriggerClientEvent("vorp_inventory:ProcessingReady", _source)
