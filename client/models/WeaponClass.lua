...
function Weapon:RemoveWeaponFromPed()
	local isWeaponAGun = Citizen.InvokeNative(0x705BE297EEBDB95D, GetHashKey(self.name))
	local isWeaponOneHanded = Citizen.InvokeNative(0xD955FEE4B87AFA07, GetHashKey(self.name))
	local move = false
		...
		for k,v in pairs(equippedWeapons) do
			if v.id == self.id then
				if #equippedWeapons > 1 then
					Citizen.InvokeNative(0x3E4E811480B3AE79, 1, v.guid, 1, GetHashKey("ADD_REASON_DEFAULT"))
					move = true
				end
	...
	if move then
		local characterItem = getGuidFromItemId(1, nil, GetHashKey("CHARACTER"), 0xA1212100)
		if not characterItem then
			return false
		end
...
function Weapon:equipwep(issame)
	local isWeaponMelee = Citizen.InvokeNative(0x959383DCD42040DA, GetHashKey(self.name))
	local isWeaponAGun = Citizen.InvokeNative(0x705BE297EEBDB95D, GetHashKey(self.name))
	local isWeaponOneHanded = Citizen.InvokeNative(0xD955FEE4B87AFA07, GetHashKey(self.name))
